...
@if ($channel->url)
    <a data-ajaxnav="2" data-ajaxnav-target="#sub-content" href="{{ $channel->url }}" class="channel-item{{ $channel->type == $type && $channel->code == $code && $channel->subcode == ($subcode ?? null) ? ' active' : '' }}">
@else
